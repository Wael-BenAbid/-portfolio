...
def compress_image_for_cloudinary(image_file, max_bytes):
    ...
    Tries JPEG quality reduction first, then dimension capping.
    Returns original file unchanged if already within limit or if GIF/failure.
    """
    ...
    try:
        ...
        logger.warning(
            "Image compression failed for '%s': %s — uploading original",
            image_file.name, exc,
        )
        image_file.seek(0)
        return image_file
...
class MediaUploadSerializer(serializers.ModelSerializer):
    ...
    def validate_file(self, value):
        ...
        if detected_mime in ALLOWED_IMAGE_MIME_TYPES and value.size > cloudinary_max:
            value = compress_image_for_cloudinary(value, cloudinary_max)
